from tartan.constants import colour_signifier_width_A3,fontSize_A3,canvas_width_A3, margin_A3, y_tartan_offset_A3_Key, label_gap_A3
...
def draw_colour_grid(drawbot, unique_colors, x, height, font_size):
  ...
  for i in range(len(unique_colors)+1):
    horizontal_y = y_tartan_offset_A3_Key - font_size - ((height/(len(unique_colors))) * i)
    vertical_x = x + ((height/(len(unique_colors))) * i)
    drawbot.line((x, horizontal_y), (x + height, horizontal_y))
    drawbot.line((vertical_x, y_tartan_offset_A3_Key - font_size), (vertical_x,  y_tartan_offset_A3_Key - font_size - height))
    if i < len(unique_colors):
      draw_horizontal_lines(vertical_x+label_gap_A3,vertical_x+(height/(len(unique_colors)))-label_gap_A3,y_tartan_offset_A3_Key - font_size - height-label_gap_A3,drawbot,i+1)
      draw_vertical_lines(x - label_gap_A3,horizontal_y-label_gap_A3,horizontal_y-(height/(len(unique_colors)))+label_gap_A3,drawbot,i+1)
...
def draw_letters_key(drawbot,stripes, width_divisor,name):
  ...
  font_size =  (y_tartan_offset_A3_Key - margin_A3) /number_of_rows
  if font_size < fontSize_A3:
    drawbot.fontSize(font_size)
  else:
    drawbot.fontSize(fontSize_A3)
  ...
  text_x_position = margin_A3 + normalize_width(get_largest_width(character_widths.keys()),get_largest_width(character_widths.keys())) + (label_gap_A3 * 4)
  colour_x_position = text_x_position + largest_text_width + (label_gap_A3 * 4)
  colour_width = canvas_width_A3 - (colour_x_position + ((number_of_rows * font_size) - font_size) + ((colour_signifier_width_A3 * 6)) - (label_gap_A3 * 3) - 3 )
  colour_x_end_position = colour_x_position + colour_width + (label_gap_A3 * 6)
  ...
  draw_colour_grid(
    drawbot,
    unique_colors,
    colour_x_end_position + colour_signifier_width_A3 * 6 + colour_signifier_width_A3,
  ...
  for width, characters in character_widths.items():
    ...
    if(len(character_set)>0):
      ...
      drawbot.text(
        ', '.join(character_set), (
          text_x_position, y_tartan_offset_A3_Key - (font_size * index)
          )
      )
      ...
      drawbot.line(
        (
          margin_A3,
          y_tartan_offset_A3_Key - (font_size * index)
        ),
        (
          margin_A3 + normalized_width, 
          y_tartan_offset_A3_Key - (font_size * index)
        )
      )
