...
def draw_outlined_tartan_A4(drawbot: drawBotDrawingTools.DrawBotDrawingTool, name):
  ...
  def draw_key_lines(x1,x2,y1,y2,color,colors):
    x_margin_80_pct = (margin_A4 - (label_gap_A4*2))
    x_margin_70_pct = (margin_A4 - (label_gap_A4*3))
    x_margin_60_pct = (margin_A4 - (label_gap_A4*4))
    
    y_margin_80_pct = (margin_A4 - (label_gap_A4*2)) + y_tartan_offset_A4
    y_margin_70_pct = (margin_A4 - (label_gap_A4*3)) + y_tartan_offset_A4
    y_margin_60_pct = (margin_A4 - (label_gap_A4*4)) + y_tartan_offset_A4
  ...
  def draw_square(stripes):
    ...
    for index, character in enumerate(stripes):
      ...
      x1 = margin_A4 + (width_so_far - width)
      y1 = margin_A4 + y_tartan_offset_A4
      
      x2 = margin_A4
      y2 = margin_A4 + (width_so_far - width) + y_tartan_offset_A4

      drawbot.rect(x1,y1,width,canvas_width_A4)
      drawbot.rect(x2,y2,canvas_width_A4,width)

      draw_key_lines(
        x1 + (label_gap_A4 / 2),
        (x2 + width_so_far)-(label_gap_A4/2),
        (y1 + width_so_far)-(label_gap_A4/2),
        y2 + (label_gap_A4 / 2),
        character['color'],
  ...
  drawbot.newPage(canvas_width_A4 + margin_A4 * 2, canvas_height_A4 + margin_A4 * 2)
  fontName = drawbot.installFont('./assets/CormorantGaramond-Light.ttf')
  drawbot.font(fontName, fontSize_A4)
  drawbot.strokeWidth(1)
  ...
  name_as_stripes = [
    {
      ...
      'width': name_widths[index] * ((canvas_width_A4 / 2) / name_width),
      'letters': [x],
  ...
  draw_name(name)
  draw_letters_key(drawbot,name_as_stripes,((canvas_width_A4 / 3) / name_width),name)